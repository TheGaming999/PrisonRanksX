...
public class RankupMax {
	...
	public void rankupMax(final Player player) {
		...
		// player values
		String nextRankDisplay = !canPrestigeMap.containsKey(name) && !canPrestige && !isLastRank
				? main.getString(prxAPI.getPlayerRankupDisplay(p), name) : "null";
		double nextRankCost = !canPrestigeMap.containsKey(name) && !canPrestige && !isLastRank
				? prxAPI.getPlayerRankupCostWithIncreaseDirect(p) : 0.0;
		...
		String rankupMessage = !canPrestigeMap.containsKey(name) && !canPrestige && !isLastRank
				? main.getString(main.messagesStorage.getStringMessage("rankup"), name) : "null";
		String rankupNoPermissionMessage = !canPrestigeMap.containsKey(name) && !canPrestige && !isLastRank
				? main.getString(main.messagesStorage.getStringMessage("rankup-nopermission"), name)
		...
		if (nextRankCost > playerBalance && !canPrestigeMap.containsKey(name) && !canPrestige && !isLastRank) {
			for (String msg : notEnoughMoneyMessage) {
				p.sendMessage(main.getString(msg, name)
						.replace("%player%", name)
						.replace("%rankup_cost%", nextRankCostInString)
						.replace("%rankup%", nextRank)
						.replace("%rankup_display%", nextRankDisplay)
						.replace("%rankup_cost_formatted%", nextRankCostFormatted));
			}
			rankupMaxProcess.remove(name);
			return;
		}
		// if per rank permission option is enabled and the player dosen't has the
		// required permission
		if (isPerRankPermission && !p.hasPermission(main.rankupCommand.getPermission() + "." + nextRank)
				&& !canPrestigeMap.containsKey(name) && !canPrestige && !isLastRank) {
		...
		main.newSharedChain("maxprocess#" + name).abortIf(!player.isOnline()).async(() -> {
			...
			for (String rankSection : ranksConfigList) {
				...
				loopNextRankCost = prxAPI.getIncreasedRankupCostX(main.playerStorage.getPlayerRebirth(p),
						main.playerStorage.getPlayerPrestige(p), main.rankStorage.getRankupCost(rp));
				...
				String loopRankupNoPermissionMessage = main
						.getString(main.messagesStorage.getStringMessage("rankup-nopermission"), p.getName())
						.replace("%nextrank%", loopNextRank)
						.replace("%rankup%", loopNextRank)
						.replace("%rankup_display%", loopNextRankDisplay);
				...
				if (loopNextRankCost > loopPlayerBalance) {
					for (String msg : notEnoughMoneyMessage) {
						p.sendMessage(main.getString(msg, name)
								.replace("%player%", name)
								.replace("%rankup_cost%", loopNextRankCostInString)
								.replace("%rankup%", loopNextRank)
								.replace("%rankup_display%", loopNextRankDisplay)
								.replace("%rankup_cost_formatted%", loopNextRankCostFormatted));
					}
					rankupMaxProcess.remove(name);
					break;
				}
				...
				// after check actions
				loopRankupMsg = main.getString(main.messagesStorage.getStringMessage("rankup"), name)
						.replace("%rankup%", loopNextRank)
						.replace("%rankup_display%", loopNextRankDisplay)
						.replace("%rankup_cost%", loopNextRankCostInString)
						.replace("%rankup_cost_formatted%", loopNextRankCostFormatted);
				if (!isRankupMsgLastRankOnly) {
					if (!loopRankupMsg.isEmpty()) p.sendMessage(loopRankupMsg);
				}
				...
				if (main.rankStorage.getAddPermissionList(rp) != null
						&& !main.rankStorage.getAddPermissionList(rp).isEmpty()) {
					permissionFuture = CompletableFuture.runAsync(() -> {
						for (String addpermission : main.rankStorage.getAddPermissionList(rp)) {
							main.perm.addPermissionAsync(p,
									addpermission.replace("%player%", name)
											.replace("%rankup%", accessibleNextRank.getString())
											.replace("%rank%", mapRank));
						}
					});
				}
				if (main.rankStorage.getDelPermissionList(rp) != null
						&& !main.rankStorage.getDelPermissionList(rp).isEmpty()) {
						...
						CompletableFuture<Void> continuePermissionFuture = permissionFuture.thenRunAsync(() -> {
							try {
								...
								for (String delpermission : main.rankStorage.getDelPermissionList(rp)) {
									main.perm.delPermissionAsync(p,
											delpermission.replace("%player%", name)
													.replace("%rankup%", accessibleNextRank.getString())
													.replace("%rank%", mapRank));
								}
				...
				if (loopNextRankBroadcast != null && !isBroadcastLastRankOnly) {
					OnlinePlayers.getPlayers().forEach(ap -> {
						...
						for (String broadcast : main.rankStorage.getBroadcast(rp)) {
							ap.sendMessage(main.getString(broadcast, name)
									.replace("%player%", name)
									.replace("%rankup%", main.rankStorage.getRankupName(rp))
									.replace("%rankup_display%", main.rankStorage.getRankupDisplayName(rp))
									.replace("%rankupdisplay%", main.rankStorage.getRankupDisplayName(rp)));
						}
					});
				}
				...
				main.debug("Ranked up to: " + loopNextRank);
			}
			// end of loop
			// save player data
			String mapRank = rankupMaxMap.get(name);
			main.debug("Final rankup: " + mapRank);
			RankPath rp = new RankPath(mapRank, main.prxAPI.getDefaultPath());
			int endNextRankActionbarInterval = 20;
			...
			boolean endIsBroadcastLastRankOnly = main.globalStorage
					.getBooleanData("Options.rankupmax-broadcastlastrankonly");
			boolean endIsMsgLastRankOnly = main.globalStorage.getBooleanData("Options.rankupmax-msglastrankonly");
			boolean endIsRankupMsgLastRankOnly = main.globalStorage
					.getBooleanData("Options.rankupmax-rankupmsglastrankonly");
					...
					OnlinePlayers.getPlayers().forEach(ap -> {
						if (!main.isInDisabledWorld(ap)) {
							for (String broadcast : broadcastList) {
								ap.sendMessage(main.getString(broadcast, name)
										.replace("%player%", name)
										.replace("%rankup%", rp.getRankName())
										.replace("%rankup_display%", main.rankStorage.getDisplayName(rp))
										.replace("%rankupdisplay%", main.rankStorage.getDisplayName(rp)));
							}
						}
					});
				}
			...
			if (endIsMsgLastRankOnly) {
				for (String msg : endNextRankMsg) {
					p.sendMessage(main.getString(msg, name)
							.replace("%player%", name)
							.replace("%rankup_display%", main.getString(main.rankStorage.getDisplayName(rp), name)));
				}
			}
			if (endIsRankupMsgLastRankOnly && !isRankupMaxMsg) {
				if (!rankupMessage.isEmpty()) p.sendMessage(rankupMessage.replace("%rankup%", mapRank)
						.replace("%rankup_display%", main.getString(main.rankStorage.getDisplayName(rp), name)));
			}
			...
			if (isRankupMaxMsg && !rankupFromMap.get(name).equals(mapRank)) {
				if (!rankupMaxMsg.isEmpty()) p.sendMessage(main.getString(rankupMaxMsg, name)
						.replace("%rank%", rankupFromMap.get(name))
						.replace("%rank_display%",
								prxAPI.c(
										prxAPI.getRankDisplay(new RankPath(rankupFromMap.get(name), rp.getPathName()))))
						.replace("%rankup%", mapRank)
						.replace("%rankup_display%",
								prxAPI.c(prxAPI.getRankDisplay(new RankPath(mapRank, rp.getPathName()))))
			...
			AsyncRankupMaxEvent x = new AsyncRankupMaxEvent(p, rankupFromMap.get(name), mapRank,
					rankupMaxStreak.get(name), rankupMaxPassedRanks.get(name), false);
			...
			Bukkit.getScheduler().runTaskLater(main, () -> {
				...
				if (main.getPrestigeMax().isProcessing(name)) main.getPrestigeMax().removeProcessingPlayer(name);
	...
	public void rankupMax(final Player player, final String rankLimit) {
		...
		String rankupNoPermissionMessage = main
				.getString(main.messagesStorage.getStringMessage("rankup-nopermission"), name)
				.replace("%nextrank%", nextRank)
				.replace("%rankup%", nextRank)
				.replace("%rankup_display%", nextRankDisplay);
		...
		main.getServer().getScheduler().runTaskAsynchronously(main, () -> {
			for (String rankSection : ranksConfigList) {
				...
				if (rrc != null && rrc.getRandomCommandsMap() != null) {
					...
					if (rrc.getCommands(randomSection) != null) {
						...
						List<String> replacedCommands = new ArrayList<>();
						for (String cmd : commands) {
							String pCMD = prxAPI.cp(
									cmd.replace("%player%", name).replace("%rankup%", prxAPI.getPlayerNextRank(p)), p);
							Bukkit.getScheduler().runTask(main, () -> {
								Bukkit.dispatchCommand(Bukkit.getConsoleSender(), pCMD);
							});
						}
			...
			if (endIsBroadcastLastRankOnly) {
				for (String broadcast : endNextRankBroadcast) {
					Bukkit.broadcastMessage(main.getString(broadcast, name)
							.replace("%player%", name)
							.replace("%rankup_display%", main.getString(main.rankStorage.getDisplayName(rp), name)));
				}
			}
			...
			if (endIsRankupMsgLastRankOnly && !isRankupMaxMsg) {
				p.sendMessage(rankupMessage.replace("%rankup%", mapRank)
						.replace("%rankup_display%", main.getString(main.rankStorage.getDisplayName(rp), name)));
			}
			...
			endNextRankActionbarInterval = main.rankStorage.getActionbarInterval(rp);
			if (isRankupMaxMsg) {
				p.sendMessage(main.getString(rankupMaxMsg, name)
						.replace("%rank%", rankupFromMap.get(name))
						.replace("%rank_display%",
								prxAPI.c(
										prxAPI.getRankDisplay(new RankPath(rankupFromMap.get(name), rp.getPathName()))))
			...
			rankupMaxPassedRanks.put(name, rankups);
			AsyncRankupMaxEvent x = new AsyncRankupMaxEvent(p, rankupFromMap.get(name), mapRank,
					rankupMaxStreak.get(name), rankupMaxPassedRanks.get(name), true);
			main.getServer().getPluginManager().callEvent(x);
			Bukkit.getScheduler().runTaskLater(main, () -> {
				if (main.isRankupMaxWarpFilter) {
					if (main.rankStorage.getPlayerCommands().containsKey(mapRank)) {
