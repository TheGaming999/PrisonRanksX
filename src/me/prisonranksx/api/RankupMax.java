				...
				for(String rankSection : ranksConfigList) {
					...
					if(main.rankStorage.getAddPermissionList(rp) != null && !main.rankStorage.getAddPermissionList(rp).isEmpty()) {
						for(String addpermission : main.rankStorage.getAddPermissionList(rp)) {
							main.debug("addPermission: " + main.perm.addPermissionAsync(p, addpermission.replace("%player%", name).replace("%rankup%", loopNextRank).replace("%rankup_display%", loopNextRankDisplay).replace("%rank%", loopCurrentRank)));
						}
					}
					if(main.rankStorage.getDelPermissionList(rp) != null && !main.rankStorage.getDelPermissionList(rp).isEmpty()) {
						for(String delpermission : main.rankStorage.getDelPermissionList(rp)) {
							main.debug("delPermission: " + main.perm.delPermissionAsync(p, delpermission.replace("%player%", name).replace("%rankup%", loopNextRank).replace("%rankup_display%", loopNextRankDisplay).replace("%rank%", loopCurrentRank)));
						}
					}
