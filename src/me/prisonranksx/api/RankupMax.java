...
public class RankupMax {
	...
	public void rankupMax(final Player player) {
        ...
        // player values
        String nextRankDisplay = !canPrestigeMap.containsKey(name) && !canPrestige ? main.getString(prxAPI.getPlayerRankupDisplay(p), name) : "null";
	    double nextRankCost = !canPrestigeMap.containsKey(name) && !canPrestige ? prxAPI.getPlayerRankupCostWithIncreaseDirect(p) : 0.0;
    	...
    	String rankupMessage = !canPrestigeMap.containsKey(name) && !canPrestige ? main.getString(main.messagesStorage.getStringMessage("rankup"), name) : "null";
    	String rankupNoPermissionMessage = !canPrestigeMap.containsKey(name) && !canPrestige ? main.getString(main.messagesStorage.getStringMessage("rankup-nopermission"), name).replace("%nextrank%", nextRank).replace("%rankup%", nextRank).replace("%rankup_display%", nextRankDisplay) : "null";
        //if the rank cost is higher than player's balance
        if(nextRankCost > playerBalance && !canPrestigeMap.containsKey(name) && !canPrestige) {
        ...
        //==============
        //@@@@@@@@@@@@@@@@@@@@
 	  main.getTaskChainFactory().newSharedChain("rumax#" + name).async(() -> {
